...
def exportar_todas_tabelas():
    ...
    pasta_output = raiz_projeto / "db" / "output"
    if not pasta_output.exists() and (raiz_projeto / "DB" / "OUTPUT").exists():
        pasta_output = raiz_projeto / "DB" / "OUTPUT"
    pasta_backup = pasta_output / f"{timestamp}_BACKUP_COMPLETO_ELABORE"
